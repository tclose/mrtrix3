...
SCRATCH_DIR = ""
VERBOSITY = (
    0 if "MRTRIX_QUIET" in os.environ else int(os.environ.get("MRTRIX_LOGLEVEL", "1"))
)
...
_DEFAULT_COPYRIGHT = """Copyright (c) 2008-2025 the MRtrix3 contributors.
...
def _execute(usage_function, execute_function):  # pylint: disable=unused-variable
    ...
    try:
        for key in vars(ARGS):
            ...
            if isinstance(
                value, Parser._UserOutPathExtras
    ...
    if _STDOUT_IMAGES and sys.stdout.isatty():
        sys.stderr.write(
            f"{EXEC_NAME}: {ANSI.error}[ERROR] Cannot pipe output images as no command connected to stdout{ANSI.clear}\n"
        )
        sys.stderr.flush()
        sys.exit(1)
    ...
    if cli_parse_only:
        try:
            ...
            warn(
                'Potentially corrupt environment variable "MRTRIX_CLI_PARSE_ONLY" '
    ...
    try:
        ...
        if SCRATCH_DIR:
            with open(
                os.path.join(SCRATCH_DIR, "error.txt"), "w", encoding="utf-8"
            ) as outfile:
                outfile.write(
                    (exception.command if is_cmd else exception.function)
                    + "\n\n"
                    + str(exception)
                    + "\n"
                )
        ...
        sys.stderr.write("\n")
        sys.stderr.write(
            f"{EXEC_NAME}: {ANSI.error}[ERROR] {exception.command if is_cmd else exception.function}{ANSI.clear} {ANSI.debug}({os.path.basename(filename)}:{lineno}){ANSI.clear}\n"
        )
        if str(exception):
            sys.stderr.write(
                f'{EXEC_NAME}: {ANSI.error}[ERROR] Information from failed {"command" if is_cmd else "function"}:{ANSI.clear}\n'
            )
            ...
            sys.stderr.write(
                f'{EXEC_NAME}: {ANSI.error}[ERROR] Failed {"command" if is_cmd else "function"} did not provide any output information{ANSI.clear}\n'
            )
        if SCRATCH_DIR:
            sys.stderr.write(
                f"{EXEC_NAME}: {ANSI.error}[ERROR] For debugging, inspect contents of scratch directory: {SCRATCH_DIR}{ANSI.clear}\n"
            )
        sys.stderr.flush()
        ...
        sys.stderr.write("\n")
        sys.stderr.write(
            f"{EXEC_NAME}: {ANSI.error}[ERROR] Unhandled Python exception:{ANSI.clear}\n"
        )
        sys.stderr.write(
            f"{EXEC_NAME}: {ANSI.error}[ERROR]{ANSI.clear}   {ANSI.console}{type(exception).__name__}: {exception}{ANSI.clear}\n"
        )
        ...
        for item in inspect.getinnerframes(traceback)[1:]:
            try:
                ...
                calling_code = item[4]
            sys.stderr.write(
                f"{EXEC_NAME}: {ANSI.error}[ERROR]{ANSI.clear}   {ANSI.console}{filename}:{lineno} (in {function}()){ANSI.clear}\n"
            )
            for line in calling_code:
                sys.stderr.write(
                    f"{EXEC_NAME}: {ANSI.error}[ERROR]{ANSI.clear}     {ANSI.debug}{line.strip()}{ANSI.clear}\n"
                )
    finally:
...
def debug(text):  # pylint: disable=unused-variable
    ...
    try:
        ...
        sys.stderr.write(
            f"{EXEC_NAME}: {ANSI.debug}[DEBUG] {origin}: {text}{ANSI.clear}\n"
        )
    finally:
        del nearest
...
def var(*variables):  # pylint: disable=unused-variable
    ...
    try:
        ...
        lineno = calling_frame.lineno
        var_string = (
            calling_code[calling_code.find("var(") + 4 :]
            .rstrip("\n")
            .rstrip(" ")[:-1]
            .replace(",", " ")
        )
        ...
        for name, value in zip(var_names, var_values):
            sys.stderr.write(
                f"{EXEC_NAME}: [{os.path.basename(filename)}:{lineno}]: {name} = {value}\n"
            )
    finally:
        del calling_frame
...
class ProgressBar:  # pylint: disable=unused-variable
    ...
    def __init__(self, msg, target=0):
        ...
        if not (isinstance(msg, str) or callable(msg)):
            raise TypeError(
                "app.ProgressBar must be constructed using either a string or a function"
            )
        self.counter = 0
        ...
        self.newline = (
            "\n" if VERBOSITY > 1 else ""
        )  # If any more than default verbosity, may still get details printed in between progress updates
        ...
        if self.isatty:
            progress_bar = (
                f"{self.value:>3}%" if self.multiplier else ProgressBar.BUSY[0]
            )
            sys.stderr.write(
                f"{self.wrapoff}{EXEC_NAME}: {ANSI.execute}[{progress_bar}]{ANSI.clear} {ANSI.console}{self._get_message()}... {ANSI.clear}{ANSI.lineclear}{self.wrapon}{self.newline}"
            )
        else:
    ...
    def done(self, msg=None):
        ...
        if self.isatty:
            ...
            sys.stderr.write(
                f"\r{EXEC_NAME}: {ANSI.execute}[{progress_bar}]{ANSI.clear} {ANSI.console}{self._get_message()}{ANSI.clear}{ANSI.lineclear}\n"
            )
        else:
            if self.newline:
                sys.stderr.write(
                    f'{EXEC_NAME}: {self._get_message()} [{"=" * int(self.value/2)}]\n'
                )
            else:
                sys.stderr.write(
                    f'{"=" * (int(self.value/2) - int(self.old_value/2))}]\n'
                )
        sys.stderr.flush()
    ...
    def _update(self):
        ...
        if self.isatty:
            progress_bar = (
                f"{self.value:>3}%"
                if self.multiplier
                else ProgressBar.BUSY[self.counter % 6]
            )
            sys.stderr.write(
                f"{self.wrapoff}\r{EXEC_NAME}: {ANSI.execute}[{progress_bar}]{ANSI.clear} {ANSI.console}{self._get_message()}... {ANSI.clear}{ANSI.lineclear}{self.wrapon}{self.newline}"
            )
        else:
            if self.newline:
                sys.stderr.write(
                    f'{EXEC_NAME}: {self._get_message()}... [{"=" * int(self.value/2)}{self.newline}'
                )
            else:
...
class Parser(argparse.ArgumentParser):
    ...
    def make_userpath_object(base_class, *args):
        ...
        new_class = type(
            f'{base_class.__name__.lstrip("_").rstrip("Extras")}',
        ...
        if sys.version_info < (3, 12):
            ...
            super(super_class, instance).__init__(
                abspath
    ...
    class _UserDirOutPathExtras(_UserOutPathExtras):
        ...
        def mkdir(self, mode=0o777):  # pylint: disable=arguments-differ
            while True:
                ...
                try:
                    super().mkdir(
                        mode, parents=True, exist_ok=False
        ...
        def __call__(self, input_value):
            ...
            try:
                ...
                raise argparse.ArgumentTypeError(
                    f'Could not interpret "{input_value}" as boolean value'
                ) from exc
        ...
        class IntBounded(Parser.CustomTypeBase):
            def __call__(self, input_value):
                try:
                    ...
                    raise argparse.ArgumentTypeError(
                        f'Could not interpret "{input_value}" as integer value'
                    ) from exc
                if min_value is not None and value < min_value:
                    raise argparse.ArgumentTypeError(
                        f'Input value "{input_value}" less than minimum permissible value {min_value}'
                    )
                if max_value is not None and value > max_value:
                    raise argparse.ArgumentTypeError(
                        f'Input value "{input_value}" greater than maximum permissible value {max_value}'
                    )
                return value
        ...
        class FloatBounded(Parser.CustomTypeBase):
            def __call__(self, input_value):
                try:
                    ...
                    raise argparse.ArgumentTypeError(
                        f'Could not interpret "{input_value}" as floating-point value'
                    ) from exc
                if min_value is not None and value < min_value:
                    raise argparse.ArgumentTypeError(
                        f'Input value "{input_value}" less than minimum permissible value {min_value}'
    ...
    class SequenceInt(CustomTypeBase):
        def __call__(self, input_value):
            try:
                ...
                raise argparse.ArgumentTypeError(
                    f'Could not interpret "{input_value}" as integer sequence'
                ) from exc
    ...
    class SequenceFloat(CustomTypeBase):
        def __call__(self, input_value):
            try:
                ...
                raise argparse.ArgumentTypeError(
                    f'Could not interpret "{input_value}" as floating-point sequence'
                ) from exc
    ...
    class DirectoryIn(CustomTypeBase):
        def __call__(self, input_value):
            ...
            if not abspath.exists():
                raise argparse.ArgumentTypeError(
                    f'Input directory "{input_value}" does not exist'
                )
            if not abspath.is_dir():
                raise argparse.ArgumentTypeError(
                    f'Input path "{input_value}" is not a directory'
                )
            return abspath
    ...
    class DirectoryOut(CustomTypeBase):
        def __call__(self, input_value):
            abspath = Parser.make_userpath_object(
                Parser._UserDirOutPathExtras, input_value
            )
            return abspath
    ...
    class FileIn(CustomTypeBase):
        def __call__(self, input_value):
            ...
            if not abspath.exists():
                raise argparse.ArgumentTypeError(
                    f'Input file "{input_value}" does not exist'
                )
            if not abspath.is_file():
                raise argparse.ArgumentTypeError(
                    f'Input path "{input_value}" is not a file'
                )
            return abspath
    ...
    class FileOut(CustomTypeBase):
        def __call__(self, input_value):
            return Parser.make_userpath_object(
                Parser._UserFileOutPathExtras, input_value
            )
    ...
    class TracksIn(CustomTypeBase):
        def __call__(self, input_value):
            ...
            if filepath.suffix.lower() != ".tck":
                raise argparse.ArgumentTypeError(
                    f'Input tractogram file "{filepath}" is not a valid track file'
                )
            return filepath
    ...
    class TracksOut(CustomTypeBase):
        def __call__(self, input_value):
            ...
            if filepath.suffix.lower() != ".tck":
                raise argparse.ArgumentTypeError(
                    f'Output tractogram path "{filepath}" does not use the requisite ".tck" suffix'
                )
            return filepath
    ...
    def __init__(self, *args_in, **kwargs_in):
        ...
        if "parents" in kwargs_in:
            for parent in kwargs_in["parents"]:
                ...
                self._external_citations = (
                    self._external_citations or parent._external_citations
                )
        else:
            ...
            script_options = self.add_argument_group(
        ...
        self._is_project = os.path.abspath(
            os.path.join(
                os.path.dirname(module_file), os.pardir, "lib", "mrtrix3", "app.py"
            )
        ...
        try:
            with subprocess.Popen(
                ...
                cwd=os.path.abspath(
                    os.path.join(os.path.dirname(module_file), os.pardir)
                ),
                ...
                self._git_version = (
                    str(self._git_version.decode(errors="ignore")).strip()
    ...
    def add_citation(self, citation, **kwargs):  # pylint: disable=unused-variable
        ...
        if kwargs:
            raise TypeError(
                "Unsupported keyword arguments passed to app.Parser.add_citation(): "
                + str(kwargs)
            )
    ...
    def add_example_usage(
        self, title, code, description=""
    ...
    def flag_mutually_exclusive_options(
        self, options, required=False
    ):  # pylint: disable=unused-variable
        assert isinstance(options, list) and isinstance(
            options[0], str
    ...
    def add_subparsers(self):  # pylint: disable=arguments-differ
        ...
        for algorithm in module.ALGORITHMS:
            algorithm_module = importlib.import_module(
                "." + algorithm, "mrtrix3.commands." + module_name
            )
    ...
    def parse_args(self, args=None, namespace=None):
        ...
        assert (
            self._synopsis
        ), "Script synopsis MUST be set in script's usage() function"
        ...
        if self._subparsers:
            for alg in self._subparsers._group_actions[0].choices:
                self._subparsers._group_actions[0].choices[alg]._check_mutex_options(
                    result
                )
        return result
    ...
    def print_citation_warning(self):
        ...
        if self._subparsers:
            ...
            for alg in self._subparsers._group_actions[0].choices:
                if alg == subparser:
                    self._subparsers._group_actions[0].choices[
                        alg
                    ].print_citation_warning()
                    return
    ...
    def error(self, message):
        for entry in sys.argv:
            if "-help".startswith(entry):
                self.print_help()
                sys.exit(0)
        if self.prog and len(shlex.split(self.prog)) == len(
            sys.argv
        ):  # No arguments provided to subparser
            self.print_help()
            sys.exit(0)
        ...
        if self._subparsers:
            for alg in self._subparsers._group_actions[0].choices:
                if alg == sys.argv[1]:
                    usage = (
                        self._subparsers._group_actions[0].choices[alg].format_usage()
                    )
                    continue
        sys.stderr.write(f"\nError: {message}\n")
    ...
    def _check_mutex_options(self, args_in):
        for group in self._mutually_exclusive_option_groups:
            ...
            if count > 1:
                ...
                sys.stderr.write(
                    f"(Consult the help page for more information: {self.prog} -help)\n\n"
                )
                sys.stderr.flush()
                sys.exit(1)
    ...
    def print_help(self, file=None):
        ...
        wrapper_args = textwrap.TextWrapper(
            width=80, initial_indent="", subsequent_indent="                     "
        )
        wrapper_other = textwrap.TextWrapper(
            width=80, initial_indent="     ", subsequent_indent="     "
        )
        if self._is_project:
        ...
        text += (
            "     "
            + bold(self.prog)
            + f': {"external MRtrix3 project" if self._is_project else "part of the MRtrix3 package"}\n'
        )
        text += "\n"
        text += bold("SYNOPSIS") + "\n"
        text += "\n"
        ...
        text += (
            wrapper_other.fill(usage).replace(self.prog, underline(self.prog), 1) + "\n"
        )
        text += "\n"
        if self._subparsers:
            text += (
                ...
                + wrapper_args.fill(
                    ...
                    + self._subparsers._group_actions[0].help
                ).replace(
                    self._subparsers._group_actions[0].dest,
    ...
    def print_full_usage(self):
        ...
        if self._subparsers:
            sys.stdout.write(
                f'ARGUMENT algorithm 0 0 CHOICE {" ".join(self._subparsers._group_actions[0].choices)}\n'
            )
        else:
            for arg in self._positionals._group_actions:
                sys.stdout.write(
                    f"ARGUMENT {arg.dest} 0 {allow_multiple(arg.nargs)} {arg2str(arg)}\n"
                )
        ...
        def print_group_options(group):
            for option in group._group_actions:
                sys.stdout.write(
                    f'OPTION {"/".join(option.option_strings)} {"0" if option.required else "1"} {allow_multiple(option.nargs)}\n'
                )
                ...
                if option.metavar and isinstance(option.metavar, tuple):
                    ...
                    nargs = (
                        1
                        if multiple == "1"
                        else (option.nargs if isinstance(option.nargs, int) else 1)
                    )
                    for _ in range(0, nargs):
                        metavar_string = (
                            ...
                            if option.metavar
                                ...
                                opt.lstrip("-") for opt in option.option_strings
                            )
                        )
                        sys.stdout.write(
                            f"ARGUMENT {metavar_string} 0 {multiple} {arg2str(option)}\n"
                        )
...
def handler(signum, _frame):
    ...
    if SCRATCH_DIR:
        if DO_CLEANUP:
            ...
            sys.stderr.write(
                f"{EXEC_NAME}: {ANSI.console}Scratch directory retained; location: {SCRATCH_DIR}{ANSI.clear}\n"
            )
    for item in _STDIN_IMAGES:
        try:
            item.unlink()
